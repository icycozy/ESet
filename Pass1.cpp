...
public:
    class iterator { 
        const ESet<Key, Compare>* container;
    public:
        bool isend;
        Key kata;
    public:
        ...
        iterator(const ESet<Key, Compare>* cont, NodePtr ptr) : container(cont), isend(1){
            if(ptr) {
                isend = 0;
                kata = ptr->data;
            }
        ...
        iterator& operator ++ () {
            ...
            while (current) {
                if (container->comp(key, current->data)) {
                    next = current;
        ...
        iterator& operator -- () {
            ...
            while (current) {
                if (container->comp(current->data, key)) {
                    prev = current;
                    current = current->right;
                } else {
                    current = current->left;
        ...
        bool operator == (const iterator& rhs) const {
            return container == rhs.container && ((isend && rhs.isend) || (!isend && !rhs.isend && !container->comp(kata, rhs.kata) && !container->comp(rhs.kata, kata)));
        }
        bool operator != (const iterator& rhs) const {
            return !(*this == rhs);
        }
    ...
    ESet(const ESet& other) : root(other.root), comp(other.comp), size_(other.size_) {
        if (root) {
            ++root->ref_count;
        }
    }
    ...
    ESet(ESet&& other) noexcept : root(other.root), comp(std::move(other.comp)), size_(other.size_) {
        other.root = nullptr;
        other.size_ = 0;
    }
